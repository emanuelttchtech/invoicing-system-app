...
if(!empty($_POST['id']))
{
    ...
    $query= $db->prepare("UPDATE `insight` SET `meta_title`=?, `meta_desc`=?, `insight_title`=?,`insight_date`= ?, `insight_content`=? ,`insight_author` = ?,`insight_img` = ? WHERE md5(id) =?");
    $query->execute(array($metaTitle,$metaDesc,$title,$date,$content,$author,$fileName,$id));
    ?>
...
$query = $db->prepare("INSERT INTO `insight`(`meta_title`,`meta_desc`, `insight_title`, `insight_date`,`insight_author`,`insight_content`, `insight_img`) VALUES (?,?,?,?,?,?,?)");
$query->execute(array($metaTitle,$metaDesc,$title,$date,$author,$content,$fileName));
?>
